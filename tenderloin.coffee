...
module.exports = (robot) ->
  ...
  robot.respond /(.*) (mic|microphone) (off|on)/i, (msg) ->
    room = msg.match[1]
    state = msg.match[3]
    authenticated_client("#{TENDERLOIN_ROOT}/rooms/#{room}/commands/microphone_#{state}")
       .get() (err, res, body) ->
  ...
  robot.respond /(.*) (cam|camera) (off|on)/i, (msg) ->
    room = msg.match[1]
    state = msg.match[3]
    authenticated_client("#{TENDERLOIN_ROOT}/rooms/#{room}/commands/camera_#{state}")
       .get() (err, res, body) ->
  ...
  robot.respond /(.*) alert/i, (msg) ->
    room = msg.match[1]
    authenticated_client("#{TENDERLOIN_ROOT}/rooms/#{room}/commands/alert")
       .get() (err, res, body) ->
  ...
  robot.respond /(.*) gist (https?:\/\/gist\.github\.com\/.*\/[0-9a-f]+)/i, (msg) ->
    ...
    authenticated_client("#{TENDERLOIN_ROOT}/rooms/#{room}/gist")
       .query(q: gist_url)
  ...
  robot.respond /(.*) sound stop/i, (msg) ->
    room = msg.match[1]
    authenticated_client("#{TENDERLOIN_ROOT}/rooms/#{room}/stop_sound")
       .get() (err, response, body) ->
